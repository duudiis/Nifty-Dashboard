...
// a progress value that may be up to a second stale. Pausing or seeking the
// embed itself just snaps back — playback belongs to the dashboard controls.
//
...
export default function WatchView() {
    ...
    // Captions are switched off once per video, via onApiChange — the module
    // loads lazily when playback starts, so onReady is too early. The flag
    // makes it a default, not a lock: the CC button still works afterwards.
    const capOffRef = useRef(false);
    ...
    useEffect(() => {
        ...
        loadYouTubeIframeAPI().then((YT) => {
            ...
            ytRef.current = new YT.Player(mount, {
                ...
                playerVars: {
                    // Native controls stay for volume/captions/quality and
                    // fullscreen, but playback is enforced from the bot's state
                    // below, so pausing or seeking the embed snaps back in sync.
                ...
                events: {
                    ...
                    // Fires when a player module (captions included) loads —
                    // the moment clearing the caption track actually works.
                    onApiChange: (e) => {
                        if (capOffRef.current) return;
                        capOffRef.current = true;
                        try {
    ...
    useEffect(() => {
        ...
        rateRef.current = 1;
        capOffRef.current = false; // a fresh video may auto-show captions again
